...
it("checks institution size and PDF extension before creating a staging ticket", async () => {
  m.maximum = 5;
  await expect(
...
it("returns only an owned path and the direct Storage TUS endpoint", async () => {
  const ticket = await beginUpload({ name: "test.pdf", size: 1024 }, user);
  expect(ticket.path).toMatch(
    /^10000000-0000-4000-8000-000000000001\/staging\/[0-9a-f-]+\/test\.pdf$/,
  );
